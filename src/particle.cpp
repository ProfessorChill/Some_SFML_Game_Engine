...
ParticleSystem::ParticleSystem(sf::Vector2u canvasSize)
    : dissolve(false), particleSpeed(100.0f), transparent(sf::Color(0, 0, 0, 0)),
      dissolutionRate(4), shape(Shape::CIRCLE), gravity(sf::Vector2f(0.0f, 0.0f)),
      startPos(
...
void ParticleSystem::fuel(int particles)
{
	for (int i = 0; i < particles; i++) {
		...
		switch (this->shape) {
			case Shape::CIRCLE: {
				std::uniform_real_distribution<> randomAngle(0.0f,
									     (2.0f * 3.14159265));
				float angle = randomAngle(gen);

				std::uniform_real_distribution<> randomAngleCos(0.0f, cos(angle));
				std::uniform_real_distribution<> randomAngleSin(0.0f, sin(angle));
...
void ParticleSystem::update(float dt)
{
	for (auto it = this->particles.begin(); it != this->particles.end(); it++) {
		...
		(*it)->drawVertex.position.x += (*it)->vel.x * dt * this->particleSpeed;
		(*it)->drawVertex.position.y += (*it)->vel.y * dt * this->particleSpeed;
		...
		if ((*it)->drawVertex.position.x > this->canvasSize.x ||
		    ...
		    (*it)->drawVertex.position.y < 0 || (*it)->drawVertex.color.a < 10) {
